...
class ENSEMBLE9(nn.Module):

    def __init__(self, freeze_bert, tokenizer, model, exp_args):
        super(ENSEMBLE9, self).__init__()
        ...
        # char layers (ensemble5 only takes character features)
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, labels_fine=None, input_pos=None, input_offs=None, input_char_encode=None, input_char_ortho=None, mode = None, args = None):
        ...
        # conv layer
        char_concat_output_reshaped = char_concat_output.reshape( char_concat_output.shape[0], char_concat_output.shape[1], char_concat_output.shape[3], char_concat_output.shape[2] )
        char_concat_output_reshaped = char_concat_output_reshaped.cuda()
        ...
        for i in range( input_char_ortho.shape[0] ):
            ...
            conv1_output = self.conv1( char_concat_output_reshaped[i].float()  )
