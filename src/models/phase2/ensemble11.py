...
class ENSEMBLE11(nn.Module):

    def __init__(self, freeze_bert, tokenizer, model, exp_args):
        super(ENSEMBLE11, self).__init__()
        ...
        # Attention on CNN ortho features 
        self.self_attention = nn.MultiheadAttention(self.conv_out_dim, 1, bias=True) # attention mechanism from PyTorch
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, labels_fine=None, input_pos=None, input_offs=None, input_char_encode=None, input_char_ortho=None, mode = None, args = None):
        ...
        # apply attention to ortho conv
        attention_applied, attention_weights = self.self_attention( batch_conv, batch_conv, batch_conv, key_padding_mask=None, need_weights=True, attn_mask = None )
        # print( attention_applied.shape )

        # concat conv output with transformer+POS embeddings
        transform_pos_char_output = torch.cat( ( sequence_output_concat, attention_applied), 2)
